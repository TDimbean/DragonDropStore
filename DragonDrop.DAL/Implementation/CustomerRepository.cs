...
namespace DragonDrop.DAL.Implementation
{
    public class CustomerRepository : ICustomerRepository
    {
        ...
        public CustomerRepository(DragonDrop_DbContext context) => _context = context;
        ...
        public IEnumerable<Customer> GetAllPaginated(int pgSize, int pgIndex, IEnumerable<Customer> custList = null)
        {
            ...
            var custs = custList == null ? _context.Customers.OrderBy(c=>c.CustomerId).Skip(pgSize * (pgIndex - 1)).Take(pgSize).ToList() :
        ...
        public async Task<bool> EmailExistsAsync(string email) 
            => (await _context.Customers.SingleOrDefaultAsync(c => c.Email == email)) != null;

        public bool PhoneExists(string phone) => _context.Customers.SingleOrDefault(c => c.Phone == phone) != null;

        public async Task<bool> PhoneExistsAsync(string phone)
            => (await _context.Customers.SingleOrDefaultAsync(c => c.Phone == phone)) != null;
        ...
        public async Task<bool> CustomerIdExistsAsync(int id) => await _context.Customers.FindAsync(id) != null;
        ...
        public async Task<string> GetPassByCustomerIdAsync(int custId)
        {
            try
            {
                var cust = await _context.Customers.SingleOrDefaultAsync(c => c.CustomerId == custId);
