...
namespace DragonDrop.DAL.Implementation
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public OrderRepository(DragonDrop_DbContext context) => _context = context;
        ...
        public IEnumerable<Order> GetAllPaginated(int pgSize, int pgIndex, IEnumerable<Order> ordList = null)
        {
            ...
            var ords = ordList == null ? _context.Orders.OrderBy(o => o.OrderId).Skip(pgSize * (pgIndex - 1)).Take(pgSize).ToList() :
        ...
        {
            StaticLogger.LogInfo(GetType(), "Repo fetching all Unprocessed Orders...");
            var ords = await _context.Orders.Where(o=>o.OrderStatusId==0).OrderBy(o => o.OrderDate).ToListAsync();
            StaticLogger.LogInfo(GetType(), ords.Count().ToString() + " records found.");
            return ords;
        }
        ...
        {
            StaticLogger.LogInfo(GetType(), "Repo fetching all Processed Orders...");
            var ords = await _context.Orders.Where(o => o.OrderStatusId == 1).OrderBy(o => o.OrderDate).ToListAsync();
            StaticLogger.LogInfo(GetType(), ords.Count().ToString() + " records found.");
            return ords;
        }
        ...
        public async Task<bool> ShippingMethodIdExistsAsync(int shipId) => await _context.ShippingMethods.FindAsync(shipId) != null;

        public async Task<bool> PaymentMethodIdExistsAsync(int payId) => await _context.PaymentMethods.FindAsync(payId) != null;

        public async Task<bool> OrderStatusIdExistsAsync(int statId) => await _context.OrderStatuses.FindAsync(statId) != null;
        ...
        {
            var ord = await _context.Orders.SingleOrDefaultAsync(o => o.OrderId == ordId);
            if (ord == null) return;
            if (ord.OrderStatusId == 0) ord.OrderStatusId = 1;
            await _context.SaveChangesAsync();
        }
        ...
        {
            var ords = await _context.Orders.Where(o => ordIds.Contains(o.OrderId)).ToListAsync();
            foreach (var ord in ords) if (ord.OrderStatusId == 0) ord.OrderStatusId = 1;
            await _context.SaveChangesAsync();
        }
