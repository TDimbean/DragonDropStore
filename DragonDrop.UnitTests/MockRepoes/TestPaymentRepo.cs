...
{
    public class TestPaymentRepo : IPaymentRepository
    {
        private TestDb _context;

        public TestPaymentRepo(TestDb context) => _context = context;
        ...
        public IEnumerable<Payment> GetAllFilteredAndPaged(decimal amount, bool smaller, int pgSize, int pgIndex, IEnumerable<Payment> payList = null)
        {
            var smlGrtText = smaller ? "under" : "over";
            StaticLogger.LogInfo(GetType(), "Repo filtering and paging Payments, according to \n amount: " + smlGrtText + " than " + amount + "\n Page Index: " + pgIndex + "; Page Size: " + pgSize);
            var pays = GetAllPaginated(pgSize, pgIndex, GetAllFiltered(amount, smaller, payList));
            StaticLogger.LogInfo(GetType(), "Repo retrieved " + pays.Count().ToString() + " records.");
            return pays;
        }
        ...
        public IEnumerable<Payment> GetAllByCustomerIdFilteredSortedAndPaged(int custId, decimal amount, bool under, int pgSize, int pgIndex, string sortBy, bool desc = false)
        {
            try
            {
                StaticLogger.LogInfo(GetType(), "Repo fetching Payments " + (under ? "under" : "over") + " " + amount.ToString("{0:0.##}") + " made by Customer:\t" + _context.Customers.SingleOrDefault(c => c.CustomerId == custId).Name + ", Sorting in " + (desc ? "descending" : "ascending") + " order by: " + sortBy + " and showing Page: " + pgIndex + " of Size: " + pgSize);
            }
            catch (NullReferenceException ex)
            {
                StaticLogger.LogWarn(GetType(), "Repo found no Customer with ID:\t" + custId + ". Details: " + ex.Message + "\nStack Trace:\t" + ex.StackTrace);
            ...
            var pays = GetAllPaginated(pgSize, pgIndex, GetAllByCustomerIdFilteredAndSorted(custId, amount, !under, sortBy, desc));
            StaticLogger.LogInfo(GetType(), "Repo retrieved " + pays.Count() + " records.");
            return pays;
        }
        ...
        public async Task CreateAsync(Payment pay)
        {
            ...
            if (_context.Customers.Find(c=>c.CustomerId==pay.CustomerId) == null)
            {
                StaticLogger.LogError(GetType(), "Repo could not register Payment because no customer with ID:\t." + pay.CustomerId + " was found.");
                return;
            }
        ...
        public async Task UpdateAsync(Payment pay)
        {
            ...
            if ((_context.Customers.Where(c => c.CustomerId == pay.CustomerId).ToList()).Count == 0)
            {
                StaticLogger.LogError(GetType(), "Repo could not update Payment because no customer with ID:\t." + pay.CustomerId + " was found.");
                return;
            }
        ...
        {
            StaticLogger.LogInfo(GetType(), "Repo fetching Payment with ID:\t" + id);
            var pay =  _context.Payments.Find(p=>p.PaymentId==id);
            if (pay == null) StaticLogger.LogWarn(GetType(), "Repo found no Payment with ID:\t" + id);
            return pay;
        }
        ...
        {
            return GetAllPaginated(pgSize, pgIndex, payList);
            /*
        ...
        {
            return GetAllFiltered(searchBy, payList);
            /*
            searchBy = searchBy.ToUpper();
            StaticLogger.LogInfo(GetType(), "Repo searching Payment records by:\t" + searchBy);
        ...
        {
            return GetAllFiltered(when, before, payList);
            /*
        ...
        {
            return GetAllFiltered(amount, over, payList);
            /*
        ...
        {
            return GetAllSorted(sortBy, desc, payList);
            /* (payList != null) return await GetAllSortedAsync(sortBy, desc);
            var pays = new List<Payment>();
            ...
            (string searchBy, int pgSize, int pgIndex, IEnumerable<Payment> payList = null)
        {
            ...
            var pays = await GetAllPaginatedAsync(pgSize, pgIndex,
                  ...
                  payList == null ? _context.Payments : payList));
            StaticLogger.LogInfo(GetType(), "Repo retrieved " + pays.Count().ToString() + " records.");
            return pays;
        }
        ...
        {
            return GetAllByCustomerId(custId);
            /*
            try
            {
        ...
        public async Task<bool> PaymentIdExistsAsync(int payId) => PaymentIdExists(payId);
            //=> await _context.Payments.FindAsync(payId) != null;

        public async Task<bool> CustomerIdExistsAsync(int custId)
            //=> await _context.Customers.FindAsync(custId) != null;
            => _context.Customers.Find(c=>c.CustomerId==custId) != null;

        public async Task<bool> PaymentMethodIdExistsAsync(int methId)
            //=> await _context.PaymentMethods.FindAsync(methId) != null;
            => _context.PaymentMethods.Find(p=>p.PaymentMethodId==methId) != null;
