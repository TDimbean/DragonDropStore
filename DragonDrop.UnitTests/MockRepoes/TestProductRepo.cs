...
namespace DragonDrop.UnitTests.MockRepoes
{
    public class TestProductRepo : IProductRepository
    {
        ...
        public TestProductRepo(TestDb context) => _context = context;

        public TestDb GetContext() => _context;
        ...
        public IEnumerable<Product> GetAllSorted(string sortBy, bool desc = false, IEnumerable<Product> prodList = null)
        {
        ...
        public bool IsUniqueBarCode(string barCode, int? prodId = null) => !_context.Products.Any(p => p.BarCode == barCode && p.ProductId != prodId);
        ...
        public async Task UpdateAsync(Product prod)
        {
            ...
            if ((_context.Products.Where(p => p.BarCode == prod.BarCode && p.ProductId != prod.ProductId).ToList()).Count() > 0)
            {
                StaticLogger.LogError(GetType(), "Repo could not create record because of duplicate Bar Code:\t" + prod.BarCode.Substring(0, 1) + "-" + prod.BarCode.Substring(1, 5) + "~~" + prod.BarCode.Substring(6, 5) + "-" + prod.BarCode.Substring(11, 1));
                return;
            }
        ...
        {
            StaticLogger.LogInfo(GetType(), "Repo fetching Product with ID:\t" + id);
            var prod = _context.Products.Find(p => p.ProductId == id);
            if (prod == null) StaticLogger.LogWarn(GetType(), "Repo found no Product with ID:\t" + id);
            return prod;
        }
        ...
        {
            return GetAllFiltered(stock, greater);
            //var smlGrtText = greater ? "more" : "less";
            //StaticLogger.LogInfo(GetType(), "Repo searching Products with " + smlGrtText + " than: " + stock + " items in stock.");
        ...
        {
            return GetAllFiltered(price, greater);
            //var smlGrtText = greater ? "over" : "under";
            //StaticLogger.LogInfo(GetType(), "Repo searching Products " + smlGrtText + " : " + price + "$.");
        ...
        {
            return GetAllSorted(sortBy, desc, prodList);
            /*
        ...
        public async Task<bool> ProductIdExistsAsync(int prodId)
            => _context.Products.Find(p=>p.ProductId==prodId) != null;

        public async Task<bool> CategoryIdExistsAsync(int catId)
            => _context.Categories.Find(c=>c.CategoryId== catId) != null;
